...
def read_throughout_evaluation():
    ...
    ax.xaxis.grid(True)
    ax.set_ylabel("Read Throughput (1000 x ROT/s)", labelpad=8, fontsize=14)
    ax.set_xlabel("Client Reading Threads", labelpad=8, fontsize=14)
    ax.yaxis.set_major_formatter(ticker.FuncFormatter(lambda y, pos: '{:.0f}'.format(y/1000)))
    ax.legend(['1', '2', '3', '4'], loc='upper left', title = 'Read Nodes', fontsize=12)
