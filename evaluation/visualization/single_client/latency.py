...
from visualization.utils import PATH, CC_DIR, EC_DIR, LOCAL_REGION, DELAYS, GOODPUTS, LINESTYLES, MARKERS
...
def latency_boxplot(df, outliers=False, interval=5, fig_size=(10, 10)):
    ...
    sns.boxplot(data=df, x="goodput", y="latency",
                hue="consistency", showfliers=outliers, order=GOODPUTS)
    ax.xaxis.grid(True)
    ...
    plt.savefig(RESULT_PATH + '/latency_boxplot' +
                ('_outliers' if outliers else '') + '.png', dpi=300)
    plt.clf()
...
def latency_average_barplot(df):
    ...
    sns.barplot(data=average_latency, x="goodput", y="latency",
                hue="consistency", width=0.8, linewidth=1.5, order=GOODPUTS)
    ax.xaxis.grid(True)
    ax.set_xlabel(ax.get_xlabel().capitalize() + " (writes/s)", labelpad=10)
    ax.set_ylabel(ax.get_ylabel().capitalize() + " (ms)", labelpad=10)
    handles, labels = ax.get_legend_handles_labels()
