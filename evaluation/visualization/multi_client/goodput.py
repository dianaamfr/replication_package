...
from visualization.utils import PATH, LOCAL_REGION
import os
import pandas as pd
...
CC_GOODPUT_PATH = PATH + '/logs/scale/goodput'
...
def get_goodput_times():
    partitions = [name for name in os.listdir(CC_GOODPUT_PATH) if os.path.isdir(os.path.join(CC_GOODPUT_PATH, name))]
    dfs = []
    for partition in partitions:
        partition_dir = os.path.join(CC_GOODPUT_PATH, partition)
