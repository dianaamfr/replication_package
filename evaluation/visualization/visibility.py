...
def visibility_evaluation():
    ...
    for delay in DELAYS:
        ...
        visibility_distribution_tables([df_ec_eu, df_cc_eu, df_ec_us, df_cc_us], delay)
...
def get_ec_visibility_times(delay):
    ...
    read_client_eu = get_data(EC_LATENCY_PATH + str(delay), 'readclient-' + LOCAL_REGION)
    read_client_us = get_data(EC_LATENCY_PATH + str(delay), 'readclient-' + REMOTE_REGION)
...
def get_cc_visibility_times(delay):
    write_client_df = get_data(CC_LATENCY_PATH + str(delay), 'writeclient-' + LOCAL_REGION)
    write_node_df = get_data(CC_LATENCY_PATH + str(delay), 'writenode-1')
    pusher_df = get_data(CC_LATENCY_PATH + str(delay), 'writenode-1-s3')
    puller_eu_df = get_data(CC_LATENCY_PATH + str(delay), 'readnode-' + LOCAL_REGION + '-s3')
    puller_us_df = get_data(CC_LATENCY_PATH + str(delay), 'readnode-' + REMOTE_REGION + '-s3')
    read_client_eu = get_data(CC_LATENCY_PATH + str(delay), 'readclient-' + LOCAL_REGION)
    read_client_us = get_data(CC_LATENCY_PATH + str(delay), 'readclient-' + REMOTE_REGION)
...
def get_time_diff_ec(df_ec):
    ...
    df_ec_diff['response_time'] = get_diff(df_ec, 'client_request', 'client_response')
    df_ec_diff['read_time'] = get_diff(df_ec, 'client_request', 'read_client_response')
    return df_ec_diff
...
def visibility_distribution_tables(dfs, delay):
    ...
    for i in range(0, len(dfs)//2, 1):
        ...
        visibility_distribution_table(df_ec_stats, df_cc_stats, titles[i] + ' (' + str(1000//delay) + ' writes/s)', files[i], delay)
...
def visibility_distribution_table(df_ec_stats, df_cc_stats, title, filename, delay):
    ...
    plt.table(cellText=df_result.values, colLabels=df_result.columns, loc='center', cellLoc='center')
    plt.axis('off')
    plt.savefig(RESULT_PATH + '/' + filename + '_' + str(delay) + '.png', dpi=300, bbox_inches='tight')
    plt.clf()
