...
from .utils import PATH, CC_DIR, EC_DIR, LOCAL_REGION, DELAYS, GOODPUTS
...
def latency_evaluation():
    ...
    for delay in DELAYS:
        ...
        df_ec_latency['goodput'] = "{:.0f}".format(1000/delay)
...
def latency_average_barplot(df):
    ...
    sns.barplot(data=average_latency, x="goodput", y="latency", hue="consistency", width=0.8, edgecolor="#2a2a2a", linewidth=1.5, order=GOODPUTS, alpha=0.8)
    ax.xaxis.grid(True)
    ax.set_xlabel(ax.get_xlabel().capitalize() + " (writes/s)", labelpad=10)
    ax.set_ylabel(ax.get_ylabel().capitalize() + " (ms)", labelpad=10)
    handles, labels = ax.get_legend_handles_labels()
