...
def latency_evaluation():
    ...
    latency_boxplot(df, outliers=True, interval=10, fig_size=(7, 20))
...
def latency_boxplot(df, outliers = False, interval = 5, fig_size = (10, 10)):
    ...
    sns.boxplot(data=df, x="goodput", y="latency", hue="consistency", showfliers=outliers)
    ax.xaxis.grid(True)
...
def latency_average_barplot(df):
    ...
    sns.barplot(data=average_latency, x="goodput", y="latency", hue="consistency", width=0.8, edgecolor="#2a2a2a", linewidth=1.5, order=['5', '10', '20'], alpha=0.8)
    ax.xaxis.grid(True)
    ax.set_xlabel(ax.get_xlabel().capitalize() + " (writes/s)", labelpad=10)
    ax.set_ylabel(ax.get_ylabel().capitalize() + " (ms)", labelpad=10)
    handles, labels = ax.get_legend_handles_labels()
...
def latency_throughput_relation(df):
    ...
    sns.lineplot(data=df, x="goodput", y="latency", hue="consistency", style="consistency", markers=['s', 'o'], dashes=[linestyles[0], linestyles[0]],
                 markersize=6, estimator=estimator_99, errorbar=None, linewidth=2, legend=False, markeredgewidth=1, markeredgecolor='#2a2a2a')
    sns.lineplot(data=df, x="goodput", y="latency", hue="consistency", style="consistency", markers=['s', 'o'], dashes=[linestyles[1], linestyles[1]],
                 markersize=8, estimator=estimator_95, errorbar=None, linewidth=2, legend=False, markeredgewidth=1, markeredgecolor='#2a2a2a')
    sns.lineplot(data=df, x="goodput", y="latency", hue="consistency", style="consistency", markers=['s', 'o'], dashes=[linestyles[2], linestyles[2]],
                 markersize=10, estimator=np.mean, errorbar=None, linewidth=2, legend=False, markeredgewidth=1, markeredgecolor='#2a2a2a')
